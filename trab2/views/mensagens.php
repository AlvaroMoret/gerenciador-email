...
<body>
    ...
    <div class="dropdown">
        <button onclick="myFunction()" class="dropbtn">Remetente</button>
        <div id="myDropdown" class="dropdown-content">
            <input type="text" placeholder="Search.." id="myInput" onkeyup="filterFunction()">
            <a href="../controlers/controlerMensagem.php?opcao=3&filtro=0">Todos</a>
            <?php 
                $usuarios = $_SESSION['vUsuarios'];
                foreach($usuarios as $usuario){
                    echo '<a href="../controlers/controlerMensagem.php?opcao=3&filtro=' . $usuario->getUsuario_id() . '">' . $usuario->getUsuario_nome() . '</a>';
                }
            ?>
        </div>
    </div>
        ...
        <?php
            ...
            if(!isset($_SESSION['vMensagens'])|| count($_SESSION['vMensagens']) == 0){
                echo "<div class='typing'>Você não possui nenhuma mensagem</div>";
            }else{
        ...
        ?>
                <a class="hidden" href="../controlers/controlerMensagem.php?opcao=6&id=<?php echo $mensagem->getMensagem_id() ?>">
                <div class="card read">
                        <h3 class="card__title">Assunto: <?php echo $mensagem->getMensagem_titulo() ?></h3> 
                        <div class="card__date">Remetente: <?php echo $mensagem->getMensagem_remetente() ?></div>    
                        <div class="exclude">
                            <a href='../controlers/controlerMensagem.php?opcao=2&pId=<?php echo $mensagem->getMensagem_id() ?>' class='excla'>
            ...
            ?>
                <a class="hidden" href="../controlers/controlerMensagem.php?opcao=6&id=<?php echo $mensagem->getMensagem_id() ?>" target="_blank">
                <div class="card">
                    <h3 class="card__title"> Assunto: <?php echo $mensagem->getMensagem_titulo() ?></h3>
                    <div class="card__date"> Remetente: <?php echo $mensagem->getMensagem_remetente() ?></div>                        
                    <div class="exclude">
